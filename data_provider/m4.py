...
def _ensure_m4_triplet(root_dir="./dataset/m4", repo_id=HUGGINGFACE_REPO):
    ...
    if hf_hub_download is None:
        expected = ["M4-info.csv", "training.npz", "test.npz"]
        missing = [name for name in expected if not os.path.exists(os.path.join(root_dir, name))]
        if missing:
            raise ImportError(
                "M4 files are missing locally and 'huggingface_hub' is not installed: " + ", ".join(missing))
    files = {
