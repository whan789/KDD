...
def require_huggingface_dataset(data_path):
    if load_dataset is None:
        raise ImportError(
            f"Dataset file not found locally: {data_path}. Install 'datasets' or set --root_path/--data_path to an existing local file.")


def require_huggingface_download(data_path):
    if hf_hub_download is None:
        raise ImportError(
            f"Dataset file not found locally: {data_path}. Install 'huggingface_hub' or place the .ts file under --root_path.")


def require_sktime():
    if load_from_tsfile_to_dataframe is None:
        raise ImportError("UEA classification datasets require 'sktime'. Install it before using --data UEA.")
...
class Dataset_ETT_hour(Dataset):
    ...
    def __read_data__(self):
        ...
        if os.path.exists(local_fp):
            ...
            require_huggingface_dataset(local_fp)
...
class UEAloader(Dataset):
    ...
    def _resolve_ts_path(self, root_path, dataset_name, flag):
        ...
        if os.path.exists(local):
            return local
        require_huggingface_download(local)
