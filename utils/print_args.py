def print_args(args):
    ...
    if hasattr(args, 'use_som'):
        seeds = ','.join(map(str, args.seeds)) if hasattr(args, 'seeds') else args.seed
        print(f'  {"Use SOM:":<20}{args.use_som:<20}{"Seeds:":<20}{seeds:<20}')
        backbone_checkpoint = args.backbone_checkpoint if getattr(args, 'backbone_checkpoint', None) else 'auto som0 setting'
        print(f'  {"Backbone Ckpt:":<20}{backbone_checkpoint:<20}')
    print()

    if hasattr(args, 'som_use_mlp_head'):
